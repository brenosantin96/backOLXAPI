...
//A funcao gera um nome para imagem, le os bytes da img, formata a imagem e salva...
...
module.exports = {
    ...
    addAction: async(req, res) => {
        //recebemos os dados, organizamos, manda imagem daquele produto ou não.
        ...
        if (!category) {
            res.json({ error: "Essa categoria não existe!" });
            return;
        }
        ...
        newAd.price = price;
        newAd.priceNegotiable = (priceneg == 'true') ? true : false; //transformando string em boolean
        newAd.description = desc;
        newAd.views = 0;

        //verificando se enviou imagens e se enviou uma ou varias
        //tem que importar biblioteca de uuid e JIMP (manipulacao de imgs)
    ...
    editAction: async(req, res) => {
        ...
        if (cat) {
            //verificar primeiro se a cat existe;
            console.log("Aqui a cat:", cat);
            const category = await Category.findOne({ slug: cat }).exec();
            if (!category) {
